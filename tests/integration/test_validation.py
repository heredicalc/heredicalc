"""Integration tests: validate FLB against COOL3 v3 reference values.

All 226 Belman pedigree permutations are parametrized from
tests/fixtures/validation_fixtures.json. Each case asserts that the
computed FLB is within ``tolerance_pct`` of ``reference_flb``.
"""
...
# Build a shared registry once at module level for speed.
...
@pytest.mark.skipif(not _CASES, reason="validation_fixtures.json not found")
@pytest.mark.parametrize(
    "case",
    _CASES,
    ids=[c["id"] for c in _CASES],
)
def test_flb_within_tolerance(case: dict[str, Any]) -> None:
    ...
    assert flb == pytest.approx(ref, rel=tol), (
        f"FLB {flb:.4f} deviates from reference {ref:.4f} "
